...
+ 用 C/C++ 实现：JVM 本身（包括解释器和即时编译器 JIT）最初主要是用 C/C++ 编写的。这使它能够直接与底层操作系统对话，并为 Java 程序提供一个统一的运行环境。

+ “像普通的 C 一样与外部交互”：这意味着 JVM 可以调用操作系统提供的 API（如 Win32 API、Linux 系统调用）。

2. Java 类库（如 java.lang.Thread）：

+ 大部分用 Java 实现：Java 标准库（jre/lib/rt.jar 等）的绝大多数功能都是用 Java 编写的，这保证了跨平台性。

+ 需要与操作系统交互的功能：当某些功能必须依赖操作系统特性时（如线程管理、文件操作、图形界面），Java 的设计是：**用 Java 代码定义一个接口，但将具体的实现交给本地的、用 C/C++ 编写的代码来完成。**

3. 本地方法（Native Method）：

+ 这就是您例子中的 setPriority0() 方法。在 Java 代码中，它使用 native 关键字声明，并且没有方法体。
...
private native void setPriority0(int newPriority); // 没有实现体
```

4. 本地方法的实现：

+ 由 JVM 直接提供：一部分最核心的本地方法（尤其是 java.* 包下的）实现是 JVM 的一部分。当 JVM 为特定平台（如 Windows）编译时，这些方法的实现就会被直接编译进去。您的例子中，Windows JVM 内部的 C 代码实现了 setPriority0，并最终调用了 Win32 的 SetThreadPriority 函数。

+ 由外部动态链接库提供：更多情况下，开发者可以编写自己的本地方法。这些方法的实现会被编译成动态链接库（Windows 上是 .dll 文件，Linux 上是 .so 文件，macOS 上是 .dylib 文件）。Java 程序在运行时通过 System.loadLibrary() 加载这些库，然后 JVM 就能找到并调用其中的函数。

**连接 Java 和本地代码的桥梁：JNI**
您描述的这个机制，其标准接口就是 **JNI**。

+ JNI 定义了一套严格的规范，规定了 Java 代码如何调用本地代码，以及本地代码如何与 JVM 内部的 Java 对象交互（例如，创建对象、调用方法、访问字段）。

+ 当您用 javac 编译一个包含 native 方法的 Java 类后，可以使用 javah（旧工具）或 javac -h（新方式）命令生成一个 C/C++ 的头文件（.h 文件）。这个头文件包含了需要实现的函数的明确定义，函数名遵循特定的命名规则（如 Java_java_lang_Thread_setPriority0）。

+ 本地代码开发者根据这个头文件实现函数，并将其编译成动态链接库。

**总结流程（以 setPriority 为例）**

1. Java 应用层：你的 Java 程序调用 myThread.setPriority(Thread.MAX_PRIORITY)。

2. Java 类库层：Thread.setPriority() 这个 Java 方法被执行，它进行参数验证等逻辑，然后调用私有的本地方法 setPriority0(MAX_PRIORITY)。

3. JNI 桥接层：JVM 接收到对 setPriority0 的调用。

4. 本地实现层：JVM 根据注册信息，找到对应的 C/C++ 函数实现（这个函数可能是 JVM 内置的）。

5. 操作系统层：该 C/C++ 函数调用操作系统的原生 API，例如在 Windows 上调用 SetThreadPriority()。

6. 返回：调用结果沿原路返回给 Java 代码。

**优点：**

+ 跨平台：Java 代码不变。对于不同的操作系统，只需要提供实现了相应 JNI 规范的本地库即可。
...
+ **丧失跨平台性：**本地库本身是平台相关的，你需要为每个目标平台编译对应的版本。

+ **安全性：**本地代码中的错误（如内存泄漏、缓冲区溢出）可能导致 JVM 崩溃，而 Java 代码通常不会。

+ **复杂性：**JNI 编程相对复杂，需要处理两种语言和环境之间的交互。

总而言之， Java 技术体系中这一关键设计思想：**通过一个稳定的接口（JNI）将可移植的 Java 代码与不可移植但功能强大、高效的本地代码分离开来。**
